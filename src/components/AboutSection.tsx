          ...
          >
            <div className="bg-background rounded-2xl border border-border p-8 shadow-card">
                ...
                {[1, 2, 3].map((i) => (
                  <div key={i} className="flex gap-4 items-start">
                    ...
                    <div className="flex-1">
                      <div className="h-3 rounded-full bg-primary/15 mb-2" style={{ width: `${85 - i * 10}%` }} />
                      <div className="h-2 rounded-full bg-muted" style={{ width: `${95 - i * 5}%` }} />
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-8 grid grid-cols-3 gap-3">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="h-24 rounded-xl bg-muted/50 border border-border flex items-center justify-center">
                    <div className="w-8 h-8 rounded-lg bg-primary/10" />
                  </div>
                ))}
              </div>
            </div>
            <div className="absolute -top-4 -right-4 w-32 h-32 rounded-full bg-secondary/15 blur-2xl" />
